...
Four more resolvers read a documentation site's own navigation file instead of a glob or a
script: they select the pages it links, and report unlinked Markdown under `scope` as residue.
All four take the same two shared options, on top of what the table lists: `path` (the
navigation file, relative to the repository root; each has its own default) and `scope` (a
glob list deciding which Markdown counts as residue when it is not linked; defaults to the
navigation file's content directory, `**/*.md`). Title and section come from the navigation
(falling back to the usual H1/frontmatter chain when it gives no title); `doc_type` comes from
the section title via one fixed heuristic (troubleshooting / tutorial / reference /
release-notes / concept, in that precedence, else `concept`).

| `resolver.type` | default `path` | resolver-specific options | notes |
|---|---|---|---|
| `vitepress` | `docs/.vitepress/config.*` | | tolerant scan of a `text`/`link`/`items` sidebar object literal (single or double quoted, escaped quotes); a link may omit `.md`, point at a directory (falls back to its `README.md`), or carry an anchor |
| `docusaurus` | `sidebars.js` or `sidebars.ts` | | the shorthand doc-id string, `{type: 'doc', id, label}`, `{type: 'category', label, items}` and `{type: 'autogenerated', dirName}`; categories become the section; ids resolve to `docs/<id>.md` or `.mdx` |
| `mdbook` | `src/SUMMARY.md` | | the nested link list; a `# Part` heading becomes the section for the links under it; a chapter with no link (`[Draft]()`) is a skipped draft, not a page or residue |
| `sitemap` | `sitemap.xml` (or a plain URL list, one per line) | `url_prefix`, `path_prefix` (required) | maps each URL's part after `url_prefix` to `path_prefix` plus the rest; `.html` becomes `.md`, and a trailing slash (or nothing left at all) falls back to `README.md` then `index.md` |
